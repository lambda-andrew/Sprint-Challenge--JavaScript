...
let contactInfo = [];
for (let i = 0; i < graduates.length; i++) {
  const string = graduates[i].first_name + " " + graduates[i].email;
...
let uniSearch = graduates.filter((uni) => {
  return uni.university === "Uni";
});
console.log(uniSearch.length);
...
let animalNames = [];
let sciName = zooAnimals.forEach(function(i){
...
let lowerCase = zooAnimals.map((lowName) => {
  return lowName.animal_name.toLowerCase();
});
...
let endangered = zooAnimals.filter((lowPop) => {
...
*/
  ...
  let newArray1 = zooAnimals.map((lowName) => {
    console.log(lowName);
      return lowName.population
    });
    console.log( newArray1);
  
  let populationTotal = newArray1.reduce((value, total) => value + total)
